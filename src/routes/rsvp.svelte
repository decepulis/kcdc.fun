<script lang="ts">
	...
	$: {
		...
		const showFam =
			categories?.includes('fam') && isNameComplete && isAttendingComplete && isAttending;
		const showFren =
			categories?.includes('fren') && isNameComplete && isAttendingComplete && isAttending;
		const showAlso = showFam && showFren;
		// Choose guest count
		const showCount = isNameComplete && isAttendingComplete && isAttending && maxGuestCount !== 1;
		const isCountComplete = guestCount !== '';
		// Chose pod members
		const showPod = isNameComplete && isAttendingComplete && isAttending && isCountComplete;
		const isPodComplete = podMembers !== '';
		// Submit button
		const showSubmit =
			isNameComplete &&
			(isNotAttending || (isAttendingComplete && isCountComplete && isPodComplete));
...
<p>
	As of the beginning of March, despite being on the decline, COVID-19 cases are still at similar
	levels to last summer's peak. We are aware that the pandemic is far from over, and we intend to
	take every precaution necessary to ensure the safety of you, our guests.
</p>
<p>
	To that end, please take the time to carefully review the following information before making an
	informed decision about your RSVP. We understand this is a challenging time and are happy to
	answer any questions you might have.
</p>
<h3>Before the Event</h3>
<p>
	To those who have any COVID-19 vaccine available to them, we hope you will consider being
	vaccinated. Please remember that vaccines take about two weeks to build full immunity. Otherwise,
	we ask that you consider taking a COVID-19 test in the days leading up to the wedding and to
	minimize contact between the test and the wedding.
</p>
<h3>At the Event</h3>
<p>
	COVID-19 is primarily transmitted via prolonged airborne contact. With that in mind, we request
	that you keep your mask on for the duration of the event, unless eating or drinking. When not
	wearing your mask, please stay with members of your own household.
</p>
<p>
	Finally, we will make an effort to hold as much of the event outdoors as possible. That being
	said, it's early-May so it's hard to predict what we'll get in terms of weather. (Past years have
	brought temperatures as low as 30&deg; and as high as 90&deg;!) Rest assured that the venue is
	well-ventilated and will be able to accomodate you with ample room if necessary.
</p>
<h2>RSVP</h2>
<Form method="POST" netlify name="rsvp" onSubmit={handleOnSubmit}>
	...
	<p class:d-none={!formState.showFren}>
		By the way, we are hoping to hold some sort of bachelor/ette party on Friday night for us
		youths. Stay tuned for more info!
	</p>
	<p class:d-none={!formState.showAlso}>Also:</p>
	<p class:d-none={!formState.showFam}>
		By the way, we plan to hold a brunch for family on Sunday morning in lieu of a rehearsal dinner.
	...
	<Button type="submit" class={`span-2 ${!formState.showSubmit ? 'd-none' : ''}`}>RSVP</Button>
</Form>
